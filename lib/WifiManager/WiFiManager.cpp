...
void WiFiManager::setupConfigPortal() {
  ...
  server->on(String(F("/wifi")), std::bind(&WiFiManager::handleWifi, this));
...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  while(1){
    ...
    if (connect) {
      connect = false;
      delay(2000);
      DEBUG_WM(F("We've received some settings"));
...
uint8_t WiFiManager::getPMChannel() {
  return _pmChannel;
}

uint16_t WiFiManager::getPMInterval() {
  return _pmInterval;
}

uint16_t WiFiManager::getPMTimeframe() {
  return _pmTimeframe;
}
...
void WiFiManager::handleWifiSave() {
  ...
  //SAVE/connect here
  _pmChannel = server->arg("s").toInt();
  _pmInterval = server->arg("p").toInt();
  _pmTimeframe = server->arg("t").toInt();
